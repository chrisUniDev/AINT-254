...
public class CheckpointDirectionHud : MonoBehaviour
{
    ...
    void PositionArrow()
    {
        ...
        if (player1)
        {
            ...
            if (m_Pos.x >= 0.0f && m_Pos.x <= 1.0f && m_Pos.y >= 0f && m_Pos.y <= 1.0f && m_Pos.z > Camera.main.nearClipPlane)
            {
        ...
        else
        {
            ...
            if (m_Pos.x >= 0.0f && m_Pos.x <= 1.0f && m_Pos.y >= 0.0f && m_Pos.y <= 1.0f && m_Pos.z > Camera.main.nearClipPlane)
            {
                m_srenderercenter.enabled = true;
                //m_Pos.x -= 0.5f; // Translate to use center of viewport 
                // m_Pos.y -= 0.5f;
