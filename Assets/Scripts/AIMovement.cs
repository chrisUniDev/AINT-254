...
public class AIMovement : MonoBehaviour {
    ...
    */
    ...
    float frontDetectionDistance = 80f;
    float rayCastOffset = 5f;
    ...
    void Update()
    {
        ...
        CheckWayPointDistance();
        //m_CashedcurrrentNode = currentNode;
        //StartCoroutine(countTimer());

        millicount();
        UpdateNodeAfterSeconds();
    }

    int m_CashedcurrrentNode;

    IEnumerator countTimer()
    {
        
        yield return new WaitForSeconds(20);
        if (m_CashedcurrrentNode == currentNode)
        {
            currentNode += 5;
        }
    }
    ...
    void Turn()
    {
        ...
        transform.rotation = Quaternion.Slerp(transform.rotation, rotation * Quaternion.AngleAxis(-(angle / 150) * 65, Vector3.forward), rotationalDamp * Time.deltaTime);
    ...
    private void millicount()
    {
        milliTime += Time.deltaTime * 10;
    }

    private void UpdateNodeAfterSeconds()
    {
        if (milliTime > 15)
        {
            milliTime = 0;
    ...
    private void CheckWayPointDistance()
    {
        if (Vector3.Distance(transform.position, nodes[currentNode].position) < 500f)
        {
