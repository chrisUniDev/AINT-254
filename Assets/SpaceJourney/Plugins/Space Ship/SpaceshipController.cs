...
public sealed class SpaceshipController : MonoBehaviour
{
	...
	private void UpdateCamera()
	{
		...
		m_lookAtPointOffset.x = Mathf.Lerp(
			...
			m_camera.LookAtPointOffset.Smooth.x * Time.smoothDeltaTime);
		...
		m_lookAtPointOffset.y = Mathf.Lerp(
			...
			m_camera.LookAtPointOffset.Smooth.y * Time.smoothDeltaTime);
		...
		m_cachedCameraTransform.rotation = Quaternion.Slerp(m_cachedCameraTransform.rotation,
			targetCameraRotation, m_camera.RotationSmooth * Time.smoothDeltaTime);
		...
		m_cachedCameraTransform.position = Vector3.Lerp(m_cachedCameraTransform.position,
			CachedTransform.position + cameraOffset, m_camera.PositionSmooth * Time.smoothDeltaTime);

		float idleCameraDistance = cameraOffset.magnitude + (cameraOffset.normalized * m_spaceship.SpeedRange.x *
			Time.smoothDeltaTime / m_camera.PositionSmooth).magnitude;
	...
	private void UpdateInput()
	{
		...
		float currentControllerSensitivity = Mathf.Lerp(m_input.Controller.Sensitivity,
			m_input.Controller.SensitivityOnMaxSpeed, SpeedFactor);

		float currentSpinSpeed = Mathf.Lerp(m_input.Controller.Sensitivity, 5.0f, SpeedFactor);
		...
		case InputMode.Controller:
			currentRawInput.x = Input.GetAxis (m_input.Controller.InputNames.ContAxisY) * currentControllerSensitivity;
			currentRawInput.y = Input.GetAxis (m_input.Controller.InputNames.ContAxisX) * currentControllerSensitivity;
			currentRawInput.z = Input.GetAxis (m_input.Controller.InputNames.ContAxisZ) * currentSpinSpeed;

			if (Input.GetAxis (m_input.Controller.InputNames.ContThrottle) != 0) {
				currentRawInput.w = Input.GetAxis(m_input.Controller.InputNames.ContThrottle) * 1.0f;
			}

			if (Input.GetAxis (m_input.Controller.InputNames.ContThrottleReduct) != 0) {
				currentRawInput.w = Input.GetAxis(m_input.Controller.InputNames.ContThrottleReduct) * -0.5f;
			}
			break;
		}
	...
	private struct InputSettings
	{
		...
		[Tooltip("Controller options.")] public ControllerSettings Controller;
	...
	[Serializable]
	private struct ControllerInputNames
	{
		[Tooltip("Rotation around x-axis (vertical movement).")] public string ContAxisX;
		[Tooltip("Rotation around y-axis (horizontal movement).")] public string ContAxisY;
		[Tooltip("Rotation around z-axis (roll).")] public string ContAxisZ;
		[Tooltip("Speed control.")] public string ContThrottle;
		[Tooltip("Speed control.")] public string ContThrottleReduct;
	}
	...
	{
		[Tooltip("Names of input axes (from InputManager).")] public ControllerInputNames InputNames;
		[Tooltip("Controller sensitivity when flying with a minimum speed.")] public float Sensitivity;
		[Tooltip("Controller sensitivity when flying with a maximum speed.")] public float SensitivityOnMaxSpeed;
	}
